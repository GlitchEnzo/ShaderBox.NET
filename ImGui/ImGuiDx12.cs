...
using Buffer = SharpDX.Direct3D12.Resource;
using Resource = SharpDX.Direct3D12.Resource;
using Device = SharpDX.Direct3D12.Device;

namespace ShaderBox
{
    // Essentially everything in DX12 is now represented by the generic Resource type as opposed to Buffer objects and ShaderResourceView objects.
    //
    // Descriptor = Represents a single shader resource
    // Descriptor Table = Represents a group (collection) of descriptors
    // Descriptor Heap = Where the Descriptor Tables are stored (for that frame?)
    // Root Signature = Represents an application binding convention that allows shaders to locate the resources they need to access
    //
    // Buffer --> Resource
    // ShaderResourceView --> Resource
    // VertexBufferBinding --> VertexBufferView
    // InputLayout --> InputLayoutDescription
    //
    // See more info about porting DX11 to DX12 here: https://docs.microsoft.com/en-us/windows/win32/direct3d12/porting-from-direct3d-11-to-direct3d-12
    public class ImGuiDx12
    {
        private int textureId;
        private IntPtr fontTextureId;
        private Dictionary<IntPtr, Resource> loadedTextures = new Dictionary<IntPtr, Resource>();

        //private ImDrawVert[] vertexData = new ImDrawVert[1024 * 16];
        private byte[] vertexData = new byte[1024 * 16 * 20]; //sizeof(ImDrawVert) = 20
        Buffer vertexBuffer;
        VertexBufferView vertexBufferBinding;
        //int vertexCount;
        ...
        public unsafe void Initialize(Device device)
        {
            ...
            inputLayout = new InputLayoutDescription(device, vertexShaderBytecode, elements);
        ...
        public IntPtr BindTexture(Texture2D texture)
        {
            ...
            var srv = new Resource(device, texture);
