...
namespace ShaderBox
{
    // Based on: https://github.com/mellinoe/synthapp/blob/master/src/synthapp/Widgets/FilePicker.cs
    // As briefly discussed here: https://github.com/mellinoe/ImGui.NET/issues/22
    public class ImGuiFilePicker
    {
        private const string FilePickerID = "###FilePicker";
        private static readonly Dictionary<object, ImGuiFilePicker> s_filePickers = new Dictionary<object, ImGuiFilePicker>();
        private static readonly Vector2 DefaultFilePickerSize = new Vector2(600, 400);

        public string CurrentFolder { get; set; }
        public string SelectedFile { get; set; }

        public static ImGuiFilePicker GetFilePicker(object o, string startingPath)
        {
            ...
            if (!s_filePickers.TryGetValue(o, out ImGuiFilePicker fp))
            {
                fp = new ImGuiFilePicker();
        ...
        public bool Draw(ref string selected)
        {
            ...
            if (ImGui.BeginPopupModal(FilePickerID, ref open, ImGuiWindowFlags.NoTitleBar))
            {
